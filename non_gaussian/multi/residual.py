...
def evaluate_residual_fusion_model(model, test_loader, scaler_Y):
    ...
    np.savez("test_data/test_dataset_onestage.npz", **test_data_arrays)
...
def plot_training_history(history):
    ...
    plt.ylabel('LR')
    plt.tight_layout()
    plt.savefig('residual_fusion_training_history_onestage.png')
    plt.close()
...
def plot_parameter_predictions(test_true, test_preds, r2_scores):
    ...
    plt.tight_layout()
    plt.savefig('residual_fusion_parameter_predictions_onestage.png')
    plt.close()
...
def plot_sample_images(model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            plt.axis('off')
        plt.tight_layout()
        plt.savefig('residual_fusion_sample_predictions_onestage.png')
        plt.close()

def main():
    print("Loading data...")
    try:
        a = np.loadtxt('E:/桌面/多模态1/Data_treat1/BTCs_InterpNew.txt')
        b = np.loadtxt('E:/桌面/多模态1/Data_treat1/DataGCSI.txt')
        X_time_series = torch.tensor(a, dtype=torch.float32)
        Y = torch.tensor(b, dtype=torch.float32)
        head_images = torch.load('E:/桌面/多模态1/processed_data1/image-3000/h-1.pt')
        k_field_images = torch.load('E:/桌面/多模态1/processed_data1/image-3000/k-1.pt')
