...
class CrossModalAttentionFusion(nn.Module):
    def __init__(self, time_series_dim, image_dim, hidden_dim=128, task="param"):
        super().__init__()
        self.task = task
        ...
        # 交叉注意力组件
        self.query_ts = nn.Linear(hidden_dim, hidden_dim)
        self.key_img = nn.Linear(hidden_dim, hidden_dim)
        self.value_img = nn.Linear(hidden_dim, hidden_dim)

        self.query_img = nn.Linear(hidden_dim, hidden_dim)
        self.key_ts = nn.Linear(hidden_dim, hidden_dim)
        self.value_ts = nn.Linear(hidden_dim, hidden_dim)

        # 输出投影（回到原始空间）
        self.out_ts = nn.Linear(hidden_dim * 2, time_series_dim)
        self.out_img = nn.Linear(hidden_dim * 2, image_dim)
    ...
    def forward(self, time_series_features, image_features):
        # 投影到共同空间
        ts_h = self.ts_projection(time_series_features)
        img_h = self.img_projection(image_features)

        batch_size = ts_h.size(0)

        # 交叉注意力：时间序列关注图像
        q_ts = self.query_ts(ts_h).unsqueeze(1)  # [batch, 1, hidden]
        k_img = self.key_img(img_h).unsqueeze(2)  # [batch, hidden, 1]
        v_img = self.value_img(img_h)  # [batch, hidden]

        # 计算注意力分数（缩放点积注意力）
        attention_scores = torch.bmm(q_ts, k_img) / math.sqrt(k_img.size(1))  # [batch, 1, 1]
        attention_weights = F.softmax(attention_scores, dim=-1)  # [batch, 1, 1]
        ts_attended_img = v_img * attention_weights.squeeze(2)  # [batch, hidden]

        # 交叉注意力：图像关注时间序列
        q_img = self.query_img(img_h).unsqueeze(1)  # [batch, 1, hidden]
        k_ts = self.key_ts(ts_h).unsqueeze(2)  # [batch, hidden, 1]
        v_ts = self.value_ts(ts_h)  # [batch, hidden]

        attention_scores = torch.bmm(q_img, k_ts) / math.sqrt(k_ts.size(1))  # [batch, 1, 1]
        attention_weights = F.softmax(attention_scores, dim=-1)  # [batch, 1, 1]
        img_attended_ts = v_ts * attention_weights.squeeze(2)  # [batch, hidden]

        # 结合原始特征和注意力特征
        ts_enhanced = torch.cat([ts_h, ts_attended_img], dim=-1)  # [batch, hidden*2]
        img_enhanced = torch.cat([img_h, img_attended_ts], dim=-1)  # [batch, hidden*2]

        # 投影回原始维度
        ts_output = self.out_ts(ts_enhanced)  # [batch, time_series_dim]
        img_output = self.out_img(img_enhanced)  # [batch, image_dim]

        # 根据任务应用alpha权重
        alpha = torch.sigmoid(self.alpha)

        if self.task == "param":
            # 对于参数预测，从时间序列开始，增强图像信息
            fused_features = time_series_features + alpha * ts_output

            # 计算贡献指标
            time_series_contrib = torch.norm(time_series_features, dim=-1) / torch.norm(fused_features, dim=-1)
            image_contrib = torch.norm(alpha * ts_output, dim=-1) / torch.norm(fused_features, dim=-1)
        else:
            # 对于图像生成，从图像特征开始，增强时间序列信息
            fused_features = image_features + alpha * img_output

            # 计算贡献指标
            image_contrib = torch.norm(image_features, dim=-1) / torch.norm(fused_features, dim=-1)
            time_series_contrib = torch.norm(alpha * img_output, dim=-1) / torch.norm(fused_features, dim=-1)

        return {
            'fused_features': fused_features,
            'alpha': alpha,
            'time_series_contrib': time_series_contrib.mean(),
            'image_contrib': image_contrib.mean()
        }
...
class PretrainedResidualFusionModel(nn.Module):
    def __init__(self, lstm_hidden_dim=128, cnn_feature_dim=256, lstm_path='best_specialized_model.pth',
        ...
        # 使用CrossModalAttentionFusion替代原有的DualResidualFusionModule
        self.param_fusion = CrossModalAttentionFusion(
            time_series_dim=self.lstm_feature_dim,
            image_dim=self.cnn_feature_dim * 4 * 4,
            hidden_dim=128,
            task="param"
        )
        self.image_fusion = CrossModalAttentionFusion(
            time_series_dim=self.lstm_feature_dim,
            image_dim=self.cnn_feature_dim * 4 * 4,
            hidden_dim=128,
            task="image"
    ...
    def generate_k_field(self, head_img, params=None, btc=None):
        ...
        if btc is not None:
            ...
            if params is None:
                param_fusion_output = self.param_fusion(lstm_features, cnn_features_flat)
                params = self.param_network(param_fusion_output['fused_features'])
        else:
...
def train_onestage_fusion_model(X_time_series, head_images, k_field_images, Y,
    ...
    # 保存标准化器
    with open("checkpoints/scaler_X_onestage.pkl", "wb") as f:
        pickle.dump(scaler_X, f)
    with open("checkpoints/scaler_Y_onestage.pkl", "wb") as f:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            ...
            save_image(comparison_grid, f'samples/onestage_epoch{epoch + 1}_comparison.png')
        ...
        if avg_val_total < best_val_loss:
            ...
            }, 'checkpoints/best_onestage_fusion_model.pth')
...
def evaluate_residual_fusion_model(model, test_loader, scaler_Y):
    ...
    np.savez("test_data/test_dataset.npz", **test_data_arrays)
...
def plot_training_history(history):
    ...
    plt.title('Cross-Attention Fusion Weights')
    plt.xlabel('Epoch')
    plt.ylabel('Weight')
    plt.legend()
    plt.subplot(3, 2, 4)
    plt.plot(history['param_time_series_contrib'], label='Param TS Contrib')
    plt.plot(history['param_image_contrib'], label='Param Img Contrib')
    plt.plot(history['image_time_series_contrib'], label='Image TS Contrib')
    plt.plot(history['image_image_contrib'], label='Image Img Contrib')
    ...
    plt.ylabel('LR')
    plt.tight_layout()
    plt.savefig('cross_attention_fusion_training_history.png')
    plt.close()
...
def plot_parameter_predictions(test_true, test_preds, r2_scores):
    ...
    plt.tight_layout()
    plt.savefig('cross_attention_fusion_parameter_predictions.png')
    plt.close()
...
def plot_sample_images(model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            plt.axis('off')
        plt.tight_layout()
        plt.savefig('cross_attention_fusion_sample_predictions.png')
        plt.close()
...
def main():
    ...
    try:
        ...
        head_images = torch.load('processed_data/image-3000/h-1.pt')
        k_field_images = torch.load('processed_data/image-3000/k-1.pt')
