...
class KFieldDecoder(nn.Module):
    ...
    def forward(self, x, skip_features=None, params=None):
        # 普通卷积网络不使用跳跃连接，因此跳过skip_features参数
        x = self.conv_up1(x)
        x = self.conv_up2(x)
        x = self.conv_up3(x)
        x = self.conv_up4(x)
...
def train_onestage_fusion_model(X_time_series, head_images, k_field_images, Y,
    ...
    # 保存标准化器
    with open("checkpoints/scaler_X_cnn.pkl", "wb") as f:
        pickle.dump(scaler_X, f)
    with open("checkpoints/scaler_Y_cnn.pkl", "wb") as f:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            ...
            save_image(comparison_grid, f'samples/cnn_epoch{epoch + 1}_comparison.png')
        ...
        if avg_val_total < best_val_loss:
            ...
            }, 'checkpoints/best_fusion_model_cnn.pth')
...
def evaluate_residual_fusion_model(model, test_loader, scaler_Y):
    ...
    np.savez("test_data/test_dataset_cnn.npz", **test_data_arrays)
...
def plot_training_history(history):
    ...
    plt.ylabel('LR')
    plt.tight_layout()
    plt.savefig('residual_fusion_training_history_cnn.png')
    plt.close()
...
def plot_parameter_predictions(test_true, test_preds, r2_scores):
    ...
    plt.tight_layout()
    plt.savefig('residual_fusion_parameter_predictions_cnn.png')
    plt.close()
...
def plot_sample_images(model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            plt.axis('off')
        plt.tight_layout()
        plt.savefig('residual_fusion_sample_predictions_cnn.png')
        plt.close()
